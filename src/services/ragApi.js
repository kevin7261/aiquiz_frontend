 ...
 *
 * Body（節錄）：rag_tab_id、person_id、unit_list；選填 unit_types（逗號字串，與 unit_list 群組對齊）、unit_type_list（整數陣列）、chunk_size／chunk_overlap（相容舊版）及 chunk_sizes／chunk_overlaps（整數陣列與群組同序）；選填 transcriptions（string[]，與 unit_list 逗號分段同序；unit_type 2／3／4 索引為 Markdown 全文 UTF-8 原樣，供寫入 Rag_Unit.transcription／transcript.md）；選填 build_faiss（true 強制允許 FAISS〔仍須 unit_type=1〕；false 等同 repack_only；省略時依使用者類型判定）。
